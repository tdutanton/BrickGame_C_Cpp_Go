...
/*
#cgo LDFLAGS: ${SRCDIR}/../../../s21_tetris.a
#include "../../tetris/s21_backend_tetris.h"
*/
import "C"
...
func (g *TetrisGame) HandleAction(actionID int, hold bool) {
	g.BaseGame.Mu.Lock()
	defer g.BaseGame.Mu.Unlock()
...
func (g *TetrisGame) GetState() (server.GameState, error) {
	g.BaseGame.Mu.Lock()
	defer g.BaseGame.Mu.Unlock()
...
func (g *TetrisGame) convertMatrix(ptr **C.int, rows, cols int) [][]bool {
	if ptr == nil {
...
func (g *TetrisGame) IsRunning() bool {
	g.Mu.Lock()
	defer g.Mu.Unlock()
	return g.Running
}
